...
def run():
    if recording:
        ...
        outLeft = cv2.VideoWriter('output/left.mp4', fourcc, fps*0.1, (width,height))  # 파일 이름, 코덱, 프레임 속도, 프레임 크기 설정
        outRight = cv2.VideoWriter('output/right.mp4', fourcc, fps*0.1, (width, height))  # 파일 이름, 코덱, 프레임 속도, 프레임 크기 설정
